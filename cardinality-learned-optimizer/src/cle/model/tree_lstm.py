...
class ChildSumTreeLSTM(nn.Module):  # type: ignore[misc]
    ...
    def _node_forward(
        ...
        if child_h.size(0) > 0:
            f = torch.sigmoid(self.W_f(x).unsqueeze(0) + self.U_f(child_h))  # [k, hidden]
...
class PlanTreeEncoder(nn.Module):  # type: ignore[misc]
