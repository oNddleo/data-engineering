...
class TestAtomicLedgerOutbox:
    ...
    def test_both_rows_created(self, payment_service, db):
        cur, keys = db
        event = PaymentEvent(sender_account="A1", receiver_account="A2", amount=Decimal("50.00"))
        keys.append(event.idempotency_key)

        payment_service.create_payment(event)

        cur.execute(
            "SELECT 1 FROM ledger WHERE idempotency_key = %s::uuid", (event.idempotency_key,)
        )
        assert cur.fetchone() is not None, "ledger row missing"

        cur.execute(
            "SELECT 1 FROM outbox WHERE idempotency_key = %s::uuid", (event.idempotency_key,)
        )
        assert cur.fetchone() is not None, "outbox row missing"

        cur.execute(
            "SELECT 1 FROM transaction_states WHERE idempotency_key = %s::uuid",
...
class TestIdempotency:
    ...
    def test_duplicate_returns_same_state(self, payment_service, db):
        cur, keys = db
        event = PaymentEvent(sender_account="B1", receiver_account="B2", amount=Decimal("25.00"))
        keys.append(event.idempotency_key)
    ...
    def test_idempotency_log_prevents_double_processing(self, db):
        ...
        cur.execute(
            "SELECT COUNT(*) AS c FROM idempotency_log"
            " WHERE idempotency_key = %s::uuid AND consumer = %s",
            (key, "warehouse"),
        )
...
class TestCoordinator:
    ...
    def test_step_advancement(self, payment_service, coordinator, db):
        cur, keys = db
        event = PaymentEvent(sender_account="C1", receiver_account="C2", amount=Decimal("10.00"))
        keys.append(event.idempotency_key)
        payment_service.create_payment(event)
    ...
    def test_completed_when_all_steps_done(self, payment_service, coordinator, db):
        cur, keys = db
        event = PaymentEvent(sender_account="D1", receiver_account="D2", amount=Decimal("99.00"))
        keys.append(event.idempotency_key)
        payment_service.create_payment(event)
    ...
    def test_failure_recorded(self, payment_service, coordinator, db):
        cur, keys = db
        event = PaymentEvent(sender_account="E1", receiver_account="E2", amount=Decimal("5.00"))
        keys.append(event.idempotency_key)
        payment_service.create_payment(event)
    ...
    def test_compensation(self, payment_service, coordinator, db):
        cur, keys = db
        event = PaymentEvent(sender_account="F1", receiver_account="F2", amount=Decimal("200.00"))
        keys.append(event.idempotency_key)
        payment_service.create_payment(event)
...
class TestOutbox:
    ...
    def test_outbox_starts_unpublished(self, payment_service, db):
        cur, keys = db
        event = PaymentEvent(sender_account="G1", receiver_account="G2", amount=Decimal("1.00"))
        keys.append(event.idempotency_key)
        payment_service.create_payment(event)
