...
class CascadesOptimizer:
    ...
    def __init__(self, catalog: StatsCatalog, cost_model: CostModel) -> None:
        ...
        self._all_predicates: list[Predicate] = []
    ...
    def _optimize_subset(self, tables: frozenset[str]) -> None:
        ...
        for size in range(1, len(tables)):
            for left_combo in itertools.combinations(table_list, size):
                ...
                algo_cost_pairs: list[tuple[PhysicalOp, str]] = [
                    (PhysicalOp.HASH_JOIN, "hash"),
                    (PhysicalOp.MERGE_JOIN, "merge"),
                    (PhysicalOp.NESTED_LOOP, "nl"),
                ]
                for algo, cost_key in algo_cost_pairs:
                    if algo == PhysicalOp.MERGE_JOIN and not preds:
                        continue  # merge join needs equi-predicate

                    local_cost: CostEstimate
                    if cost_key == "hash":
                        local_cost = self._cost_hash(left_group, right_group, out_rows)
                    elif cost_key == "merge":
                        local_cost = self._cost_merge(left_group, right_group, out_rows)
                    else:
    ...
    def extract_plan(self, winner: Winner, depth: int = 0) -> list[str]:
        ...
        for _child_gid, child_winner in winner.child_winners.items():
