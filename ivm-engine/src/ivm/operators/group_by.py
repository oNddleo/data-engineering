...
class GroupByOperator(Operator):
    """Incrementally maintained GROUP BY with configurable aggregates.
    ...
    def __init__(self, key_columns: list[str], aggregates: dict[str, Aggregate]) -> None:
        super().__init__()
    ...
    def process(self, updates: list[Update]) -> list[Update]:
        """Incrementally maintain GROUP BY aggregates for each update."""
        out: list[Update] = []
