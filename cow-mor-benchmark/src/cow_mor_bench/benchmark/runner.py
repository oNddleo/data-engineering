...
class BenchmarkSuite:
    ...
    def add(self, result: BenchmarkResult) -> None:
        result.read_speedup_cow = result.cow_trace.total_read_s / max(
            result.mor_trace.total_read_s, 1e-9
        )
        result.write_speedup_mor = result.mor_trace.total_write_s / max(
            result.cow_trace.total_write_s, 1e-9
        )
        cow_bytes = (
            result.cow_final_stats.total_data_bytes + result.cow_final_stats.total_delta_bytes
        )
        mor_bytes = (
            result.mor_final_stats.total_data_bytes + result.mor_final_stats.total_delta_bytes
        )
