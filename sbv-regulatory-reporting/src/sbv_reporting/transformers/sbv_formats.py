...
class SBVTransformer:
    ...
    def build_balance_report(
        self, df: pd.DataFrame, report_date: str | None = None
    ) -> pd.DataFrame:
        """B01-TCTD — Balance and Volume Summary by branch and currency."""
        ok = df[df["status"] == "SUCCESS"].copy()

        grp = (
            ok.groupby(["branch_code", "currency", "transaction_type"])
            .agg(
        ...
        grp.insert(
            3,
            "NGAY_BAO_CAO",
            report_date or pd.Timestamp.today().strftime(self.fmt["date_format"]),
        )
    ...
    def build_large_value_report(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        lv = df[
            (df["status"] == "SUCCESS") & (df["vnd_equivalent"] >= threshold)
        ].copy()
        ...
        report = pd.DataFrame(
            {
                ...
                "CO_VUOT_NGUONG": (lv["vnd_equivalent"] >= threshold)
                .astype(int)
                .values,
            }
        )
    ...
    def build_str_report(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        hf_accounts = set(freq.loc[freq["count"] >= freq_limit, "debit_account"])
        ...
        r4 = ok[
            (ok["vnd_equivalent"] >= lower) & (ok["vnd_equivalent"] < threshold)
        ].copy()
        r4["QUITAC"] = "R4"
